...
;; This contract allows minting and transferring of a simple fungible token with a fixed name,
...
(define-public (transfer
  ...
  (let ((sender-entry (default-to { balance: u0 } (map-get? balances { user: sender }))))
    (begin
      ...
      (map-set balances { user: recipient } { balance: (+
        (get balance
          (default-to { balance: u0 } (map-get? balances { user: recipient }))
        )
        amount
      ) }
      )
